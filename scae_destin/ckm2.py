...
from convnet_old import ConvNetLayer
...
#                                  borrow=True);
  ...
  def ckm_updates(self):
    ...
    # activation_maps=T.cast(T.ge(fms, 0.5*T.max(fms, axis=(0), keepdims=True)), dtype="float32");
    # activation_maps=fms
    activation_maps=T.cast(T.max(fms, axis=(0), keepdims=True), dtype="float32");
    ...
    # x = T.tensor4('x')
    # ams_sum=x/T.cast(T.sum(x, axis=(0,2,3), keepdims=True), dtype="float32")
    # f=theano.function([x], ams_sum)

    # # print self.filters.get_value().shape
    # temp = T.cast(update_out,'float32')
    # # print type(temp)
    # x1 = T.tensor4('x1')
    # result = x1 + update_out
    # f = theano.function([x1], result)
    # print type(f(self.filters.get_value()))
    # update_out += theano.shared(update_out)
    # update_out/=(ams_sum+1);
    #T.true_div(update_out, ams_sum);
    
    updates=[(self.filters, 
             (self.filters+update_out)/T.cast(T.sum(self.filters+update_out, axis=(0,2,3), keepdims=True), dtype="float32"))]
